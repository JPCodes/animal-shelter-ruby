...
get('/animals_customers') do
  @adopted_animals = Animal.find_adopted()
  erb(:animals_customers)
end

post('/adopted') do
  @current_animal = Animal.find_by('id', params.fetch('animal_id').to_i)
  @current_animal.adopt(params.fetch('customer_id').to_i)
  erb(:adopted)
end
